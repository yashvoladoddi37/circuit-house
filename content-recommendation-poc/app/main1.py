...
# @app.get("/recommendation/genrebasedshows")
# async def get_genre_based_shows(genre: str = Query(..., description="The genre of the TV shows to filter by", example="Drama"), limit: int = Query(20, description="The number of results to return", example=10)):
#     try:
#         top_shows = genre_based_shows(genre, limit)
...
class WeekEnum(int, Enum):
    W1 = 1
    W2 = 2
    

@app.get("/recommendation/genrebasedmovies")
async def get_genre_based_movies(genre: str = Query(..., description="The genre of the movie to filter by", example="Drama"), limit: int = Query(20, description="The number of results to return", example=10)):
    try:
        top_movies = genre_based_movies(genre, limit)
...
async def get_movie_based_movies(title: str = Query(..., description="The title of the movie to base recommendations on", example="Inception"), limit: int = Query(20, description="The number of results to return", example=10)):
    try:
        ...
        # Add logging to debug the type and content of top_movies
        logging.info(f"Type of top_movies: {type(top_movies)}")
        logging.info(f"Content of top_movies: {top_movies}")
...
async def get_top_movies(
    country: CountryEnum = Query(..., description="The country code to filter the trending movies (e.g., 'US', 'IN')", example="US"),
    week: int = Query(..., description="The week number to filter the trending movies (e.g., '32' for the 32nd week of the year)", example=32),
    limit: int = Query(20, description="The number of results to return", example=10),
    sort_by: SortByEnum = Query("popularity", description="Sort results by: popularity, rating, or release_date")
):
    try:
        logger.info(f"Parameters - country: {country}, week: {week}, limit: {limit}, sort_by: {sort_by}")
        top_movies = get_trending_movies_week(country.value, week, limit, sort_by.value)
        top_movies = top_movies.replace({np.nan: None, np.inf: None, -np.inf: None})
        top_movies_json = top_movies.to_dict(orient='records')
        return top_movies_json
    except Exception as error:
